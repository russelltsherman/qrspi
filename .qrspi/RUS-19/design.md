...
- **Body under 500 lines / 5000 tokens** — the body holds primary guidance only; depth is offloaded to `references/`. The budget is checked manually at authoring time (a line count / token estimate by inspection) and via the skill-creator eval (ref: Q5, Q11).
...
**Modified files:** none. The skill is self-contained and needs no `.claude/agents/` file (ref: Q9). The skill is **not** indexed in `README.md` or `.claude/CLAUDE.md` — those catalogs are not updated for this skill.

**Tests/queries:** none. This ticket delivers a markdown skill (`SKILL.md` + five `references/*.md`); it adds no scripts. The mechanical acceptance criteria (frontmatter validity, line count, five-doc presence) are verified manually at authoring time and via the out-of-scope skill-creator eval (ref: Q11).
...
**Decision:** Verify the three mechanically-checkable acceptance criteria (frontmatter validity, line/token budget, five-doc presence) **manually at authoring time** plus the out-of-scope skill-creator eval.

**Rationale:** This ticket delivers a markdown skill, not tooling. Authoring is done with the out-of-scope `skill-creator` skill, whose own eval loop is the intended verification path (ref: Q7, Q10); the body budget and five-doc presence are confirmed by inspection at authoring time. This matches the repo's "real validation = manual e2e" reality (ref: Q10) and keeps scope to the skill the ticket asked for.
**NEW PATTERN?** No — matches the repo's "real validation = manual e2e" reality (ref: Q10).
...
| Body exceeds 500-line/5000-token budget with no in-repo enforcement | med | med | Offload all depth to `references/` (Decision 2); enforce the line/token budget manually at authoring time and via the skill-creator eval (Decision 3) (ref: Q5, Q11). |
