export const meta = {
  name: 'qrspi-batch',
  description: 'Drive every assigned Selected / Design Approved / Plan Approved Linear ticket through QRSPI by spawning the typed phase agents from the workflow script itself',
  whenToUse: 'After moving a batch of tickets to Selected, Design Approved, or Plan Approved across any Linear project. Runs the autonomously-runnable states; the human review gates (Design Review, Plan Review) are left untouched.',
  phases: [
    { title: 'Query', detail: 'List Selected + Design Approved + Plan Approved tickets assigned to me' },
    { title: 'Setup', detail: 'Per-ticket worktree + ticket fetch (worker agent)' },
    { title: 'Design', detail: 'Spawn questions/research/design phase agents → Design Review' },
    { title: 'Plan', detail: 'Spawn structure/plan/worktree phase agents → Plan Review' },
...
// Why this exists
// ---------------------------------------------------------------------------
// A prior implementation of this workflow ran `agent("…follow qrspi-work SKILL.md…")`.
// That subagent
...
const input = typeof args === 'string'
  ...
  : args
const STATUSES = input?.statuses ?? ['Plan Approved', 'Design Approved', 'Selected'] // most-advanced first
// NOTE: the human review gates (Design Review, Plan Review) are intentionally NOT queried —
// they are human turns. The batch only drives the autonomously-runnable states: Selected
// (design half), Design Approved (plan half), Plan Approved (implementation).
...
// ===========================================================================
// SHARED SETUP  (worktree + ticket fetch + artifact detection)
// ===========================================================================
async function doSetup(t) {
  phase('Setup')
  return await agent(
...
// ===========================================================================
// DESIGN HALF  (Backlog / Selected → Design Review)
// Produces questions, research, design — then stops at the Design Review gate.
// Structure/Plan/Work Tree are produced later by runPlan, only after a human
// approves the design (status → Design Approved).
// ===========================================================================
async function runDesign(t) {
  const setup = await doSetup(t)
  if (!setup || !setup.ok) {
...
TEMPLATE_PATH = ${tpl(wd, 'design.md')}`, ctx)) return failTicket(t)
  ...
  const fin = await agent(
    `You are the DESIGN-HALF finalize worker for QRSPI ticket ${t.id}. Work inside ${wd}.

1. Verify the three design-half artifacts exist and are non-empty under ${wd}/.qrspi/${t.id}/: questions.md, research.md, design.md. If any is missing/empty, return ok:false with which — do NOT commit or change Linear.
2. Follow the "Submit and transition (design review)" steps of ${SKILL} to commit the design-half artifacts as the single planning commit (create it with the first artifact, amend for the rest), submit the planning PR with Graphite, and transition the Linear ticket to "Design Review" with a phase-transition comment.
...
Return: ok, newStatus (re-query with mcp__linear-russelltsherman__get_issue_status — expect "Design Review"), prUrl, summary (1-2 sentences).`,
  ...
  if (!fin || !fin.ok) {
    log(`  ${t.id}: design finalize failed — ${fin?.error ?? 'no result'} (artifacts NOT advanced; nothing fabricated)`)
    return { ticketId: t.id, newStatus: t.status, summary: `Design finalize failed: ${fin?.error ?? 'unknown'}` }
  }
  return { ticketId: t.id, newStatus: fin.newStatus, summary: fin.summary, prUrl: fin.prUrl }
}

// ===========================================================================
// PLAN HALF  (Design Approved → Plan Review)
// Produces structure, plan, work tree by amending the existing planning commit.
// ===========================================================================
async function runPlan(t) {
  ...
  // Guard: the design half must be complete before the plan half runs. The status is
  // Design Approved, so the human approved a design that must already be on disk.
  if (!(setup.existing.questions && setup.existing.research && setup.existing.design)) {
    log(`  ${t.id}: design-half artifacts missing — cannot run plan half (no fabrication)`)
    return { ticketId: t.id, newStatus: t.status, summary: 'Design-half artifacts missing; plan half not run.' }
  }
...
TEMPLATE_PATH = ${tpl(wd, 'worktree.md')}`, ctx)) return failTicket(t)
  ...
  phase('Finalize')
  const fin = await agent(
    `You are the PLAN-HALF finalize worker for QRSPI ticket ${t.id}. Work inside ${wd}.
...
Return: ok, newStatus (re-query with mcp__linear-russelltsherman__get_issue_status — expect "Plan Review"), prUrl, summary (1-2 sentences).`,
  ...
  if (!fin || !fin.ok) {
    log(`  ${t.id}: plan finalize failed — ${fin?.error ?? 'no result'} (artifacts NOT advanced; nothing fabricated)`)
    return { ticketId: t.id, newStatus: t.status, summary: `Plan finalize failed: ${fin?.error ?? 'unknown'}` }
  }
  return { ticketId: t.id, newStatus: fin.newStatus, summary: fin.summary, prUrl: fin.prUrl }
}
...
const DESIGN_STATES = new Set(['Selected', 'Backlog'])
...
for (let i = 0; i < tickets.length; i++) {
  ...
  let r
  if (DESIGN_STATES.has(t.status)) r = await runDesign(t)        // design half → Design Review
  else if (t.status === 'Design Approved') r = await runPlan(t)  // plan half → Plan Review
