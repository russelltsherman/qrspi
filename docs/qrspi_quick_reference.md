...
├─────────────────────────────────────────────────────────────────────┤
│ PHASE 0: TICKET (T) — 10-20 minutes                                 │
│ └─ Input: A sentence or two describing the feature                  │
│ └─ Output: ticket.md with title, description, AC, constraints       │
│ └─ Purpose: Produce a well-formed ticket via guided conversation    │
│ └─ Validation: ACs are testable; constraints are real, not invented │
│                                                                     │
...
TOTAL ALIGNMENT: 2.5–3 hours (add 10-20 min for Ticket phase)
...
```txt
After PHASE 0 (Ticket):
  ☐ Title is ≤80 chars and clearly states the feature
  ☐ At least 2 testable acceptance criteria
  ☐ Constraints and Out of Scope are explicit (not empty unless truly none)
  ☐ ticket.md written to .qrspi/<ticket-id>/
...
Install skills / set up repo  Claude Code Guide
Train an agent on QRSPI       Practical Application (Part 3)
...
```bash
# Start Phase 0 (Ticket)
/qrspi-ticket <brief description>

# Start Phase 1 (Questions)
/qrspi-questions <ticket-id>

# Start Phase 2 (Research) — always start fresh
/clear
/qrspi-research <ticket-id>

# Validate artifact
grep "should" .qrspi/<ticket-id>/research.md   # Should be empty
wc -w .qrspi/<ticket-id>/research.md            # Check size (2K-4K words)

# Check for new architecture introduced in plan
grep "class\|interface\|enum" .qrspi/<ticket-id>/design.md | sort > /tmp/d.txt
grep "class\|interface\|enum" .qrspi/<ticket-id>/plan.md | sort > /tmp/p.txt
...
Frame it or print it for your desk. The 9 phases should be muscle memory:
